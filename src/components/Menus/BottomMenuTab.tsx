        ...
        >
            <View style={{ zIndex: 100 }}>
                ...
                <View style={BottomMenuTabStyles.mainButtonRow}>
                    <Pressable onPress={() => (setIndex(0))} disabled={ showButtons ? true : false } style={{ flexDirection: "row" }}>
                        <Icon name={ index === 0 ? "calendar-text" : "calendar-text-outline"} size={index === 0 ? 30 : 25} color="white" style={{ opacity: showButtons ? 0.5 : 1, padding: 5, overflow: "hidden" }}/>
                        {index === 0 && <Text style={{ color: "white", fontSize: 15, alignSelf: "center", justifyContent: "center" }}>{"Calendar"}</Text>}
                    </Pressable>
                    <Pressable onPress={() => (setIndex(1))} disabled={ showButtons ? true : false } style={{ flexDirection: "row" }}>
                        <Icon name={ index === 1 ? "home" : "home-outline"} size={index === 1 ? 30 : 25} color="white" style={{ opacity: showButtons ? 0.5 : 1, padding: 5, overflow: "hidden" }}/>
                        {index === 1 && <Text style={{ color: "white", fontSize: 15, alignSelf: "center", justifyContent: "center" }}>{"Home"}</Text>}
                    </Pressable>
                    ...
                    <Pressable onPress={() => (setIndex(2))} disabled={ showButtons ? true : false } style={{ flexDirection: "row" }}>
                        <Icon
                            name={ index === 2 ? "text-box-search" : "text-box-search-outline"} size={index === 2 ? 30 : 25} color="white" style={{ opacity: showButtons ? 0.5 : 1, padding: 5, overflow: "hidden" }}/>
                        {index === 2 && <Text style={{ color: "white", fontSize: 15, alignSelf: "center", justifyContent: "center" }}>{"Explore"}</Text>}
                    </Pressable>
                    <Pressable onPress={() => (setIndex(3))} disabled={ showButtons ? true : false } style={{ flexDirection: "row" }}>
                        <Icon name={ index === 3 ? "heart-multiple" : "heart-multiple-outline"} size={index === 3 ? 30 : 25} color="white" style={{ opacity: showButtons ? 0.5 : 1, padding: 5, overflow: "hidden" }}/>
                        {index === 3 && <Text style={{ color: "white", fontSize: 15, alignSelf: "center", justifyContent: "center" }}>{"Analysis"}</Text>}
                    </Pressable>
                </View>
            </View>
        </KeyboardAvoidingView>
